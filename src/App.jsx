...
export default function App() {
  ...
  if (loading) return <div className="flex h-screen items-center justify-center text-gray-500 animate-pulse">Cargando Argos...</div>;
    ...
    <div className="flex h-screen bg-white font-sans text-sm overflow-hidden text-gray-800">
      ...
      <div className="w-64 bg-slate-900 text-white flex flex-col flex-shrink-0 border-r border-slate-700 z-20">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          <div className="text-xs font-semibold text-slate-500 uppercase px-2 py-2">Tableros</div>
          {boards.map(board => (
            <div 
              key={board.id}
          ...
          {isAdmin && (
            <button 
              ...
              className="flex items-center gap-2 px-3 py-2 text-slate-400 hover:text-white hover:bg-slate-800 w-full rounded mt-2 transition-colors border border-dashed border-slate-700"
            >
              <Plus size={14} />
            ...
            <div className="flex-1 overflow-auto p-6">
              <div className="bg-white rounded-lg shadow border border-gray-200 min-w-max pb-10">
                ...
                <div className="flex border-b border-gray-200 bg-gray-50 sticky top-0 z-10">
                  ...
                  <div className="w-20 p-3 text-xs font-bold text-gray-500 text-center uppercase">Chat</div>
                </div>
                ...
                {isAdmin && (
                  <div className="flex border-t border-gray-100 hover:bg-gray-50 cursor-pointer" onClick={() => addRow(activeBoard)}>
                    <div className="w-10 border-r border-gray-100 bg-blue-500"></div>
                    <div className="flex-1 p-2 pl-4 text-gray-500 flex items-center gap-2 hover:text-blue-600 text-sm">
                      <Plus size={16} />
...
function Row({ row, columns, isAdmin, onUpdate, onOpenComments }) {
  return (
    <div className="flex border-b border-gray-100 group hover:bg-blue-50/10 transition-colors bg-white">
      <div className="w-10 border-r border-gray-100 p-3 flex items-center justify-center bg-gray-50 group-hover:bg-blue-100">
      ...
      {columns.map(col => (
        <div key={col.id} className={`${col.width || 'w-40'} border-r border-gray-100 flex-shrink-0`}>
          <Cell type={col.type} value={row.values[col.id]} isAdmin={isAdmin} onChange={(val) => onUpdate(col.id, val)} />
        </div>
      ))}
...
function Cell({ type, value, onChange, isAdmin }) {
  if (type === 'status') {
    ...
    if (!isAdmin) return <div className={`m-1 h-8 flex items-center justify-center rounded text-xs font-bold text-white ${current.class}`}>{current.label}</div>;
  ...
  return isAdmin ? (
    ...
    <div className="w-full h-full px-3 flex items-center text-sm">{value}</div>
  );
}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white">
        {row.comments?.map(c => (
          <div key={c.id} className="bg-gray-100 p-3 rounded text-sm">
            <div className="flex justify-between mb-1"><span className="font-bold text-xs">{c.author}</span><span className="text-[10px] text-gray-400">{new Date(c.timestamp).toLocaleTimeString()}</span></div>
            {c.text}
          </div>
        ))}
        <div ref={endRef} />
